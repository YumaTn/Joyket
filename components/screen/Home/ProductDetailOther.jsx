...
const ProductDetailOther = ({ route, navigation }) => {
  ...
  const addToFavorites = async () => {
    try {
      ...
      const token = parsedUserData.token;
      const email = parsedUserData.email;
      const userId = parsedUserData.id; // Assuming userId is stored here
  ...
  if (!dateString) {
    return "N/A"; // Hoặc bất kỳ giá trị mặc định nào bạn muốn hiển thị nếu không có enteredDate
  }
...
const handleSelect = () => {
  if (onSelect) {
      onSelect();  // Gọi hàm onSelect để cuộn lên trên cùng
  }
};
  return (
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        {product ? (
          <>
            ...
            Ngày nhập: <Text>{formatDate(product.enteredDate)}</Text>
          </Text>
          <Text style={styles.extraInfo}>
