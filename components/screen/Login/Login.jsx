...
const Login = ({ navigation }) => {
    ...
    const fetchUserId = async (userEmail) => {
        try {
            const response = await axios.get(`http://192.168.1.16:8080/api/auth/email/${userEmail}`);
    ...
    const handleLogin = async () => {
        try {
            const response = await axios.post('http://192.168.1.16:8080/api/auth/signin', {
                email,
                password,
            });
            ...
            // Lấy userId từ API
            const userIdResponse = await axios.get(`http://192.168.1.16:8080/api/auth/email/${email}`);
